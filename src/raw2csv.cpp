...
int main(int argc, char *argv[]) {
    std::string in_raw_file_path, output_file_name;

    const std::string program_desc(
        "Code sample demonstrating how to use Metavision SDK Driver and Core to convert a RAW file to a CSV formatted file.\n");
    ...
    options_desc.add_options()
        ...
        ("out-file,o", po::value<std::string>(&output_file_name)->required(), "Output (csv) file name.");
    ...
    std::string filename(output_file_name + ".csv");
    ...
    csv_stage_ptr->set_consuming_callback([&lines, &ofs](const boost::any &data) {
        try {
            ...
            for (auto &event : *buffer) {
                ...
                ofs <<  event.t << " " << event.x << " " << event.y << " " << event.p << "\n";
            }
    ...
    log << "\r Wrote CSV file " << filename << std::endl;
